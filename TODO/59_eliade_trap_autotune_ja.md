...
### 4.5 パルサーの使い所と制約（2026-07-13）

**制約**: ELIADE のパルサーは**デジタイザ直結・固定振幅・固定レート**（preamp テスト入力は
使えない）。このため preamp 系の用途 — ENC 分解（電子ノイズ vs 電荷収集）、振幅掃引による
閾値以下の絶対トリガー効率、リニアリティ較正、レート/pileup 試験 — は**すべて不可**。

その上で残る用途:

1. **Phase 1 検証の加速**: 同一波形の繰返しは per-event 残差 ±1 LSB 基準に理想的
   （統計的ばらつきがほぼ消え、残るのは実装ミスマッチだけ）。パルス形状が preamp の指数減衰と
   違っても検証は厳密に成立する（FW と SW に同じサンプル + 同じ param を食わせた一致判定なので）。
2. **非同期位相スキャン（唯一の新規価値）**: パルサーはサンプリングクロックと非同期なので、
   パルス毎にサブサンプル位相が変わる = **無料の位相スキャン**。同一波高に対する FW energy の
   散り幅 = フィルタのサンプリング位相感度 + トリガージッタの energy への漏れ。
   **flat-top 候補値ごとにこの幅を測れば、§5.3 の trap×trigger 交差項（flat-top を詰めすぎると
   ジッタが分解能に出る）を検出器なしで直接観測できる。**
3. **TRG-IN 周期トリガ源**（パルサー信号を ch に入れない使い方）: トリガー出力 → TRG-IN で
   全 ch の記録を強制 → 検出器+preamp が接続された ch の**非バイアス純 baseline サンプル** =
   §6b.4 ノイズランの実現手段。
4. **タイミング基準（§6b.3）・ボード間共通アンカー（§7）は無傷**（固定振幅/レートで成立）。

**ENC 分解の代替**（パルサー不要）: capture run 内の複数 γ ピークで FWHM² vs E をプロットし、
E→0 外挿の切片 ≈ 電子ノイズ + 量子化。厳密さは落ちるがチューニングの解釈には十分。
...
- **ノイズラン** — random/software/pulser トリガ, 純 baseline → σ_TTF, 誤トリガ率。
  ...
  実現手段: パルサートリガー出力 → TRG-IN 強制記録、§4.5 項目 3。）
...
### 9.5 波形弁別（PSD）軸 — Si 流粒子識別の HPGe 翻訳（2026-07-13 追加）

Si の PSA（FAZIA 流 rise-time vs E の Z/A 識別）は「荷電粒子が外から入って止まる」前提なので
HPGe には移らない（エンドキャップ + デッドレイヤーで有感体積に届かない、クローバー = 非分割
同軸で位置感度も鈍い）。**ただし HPGe 固有の弁別軸が 4 つあり、②は ELI-NP 固有の価値**:

1. **SSE/MSE → ソフトウェア Compton 抑制**: HPGe PSD の本流（GERDA/Majorana A/E）。
   γ 分光への翻訳 = MSE らしさで Compton 連続成分を抑制。BEGe での ML 実証あり
   （arXiv:2412.08750、CNN peak-to-Compton 0.238→0.547 = NIM A 2024、§9.6）。BGO シールドの
   立体角の穴を埋める「第二 Compton サプレッサ」。同軸結晶は point-contact より切れない点は覚悟。
2. **中性子イベントタグ（ELI-NP 固有・本命）**: γ ビーム運転は (γ,n) 中性子環境が不可避。
   Ge(n,n'γ) 三角ピーク（596/692 keV sawtooth）+ recoil の「超局所高電離 + 脱励起 γ」という
   特異な付着形状を波形分類でタグ → **ビーム時バックグラウンド浄化**。訓練ラベル設計が課題
   （中性子源ラン vs γ 源ランの対比、三角ピーク領域の弱ラベル等）— それ自体が研究テーマ。
3. **slow-pulse（表面/n+ 層近傍）除去**: 低エネルギー側テールの掃除 = 実効ラインシェイプ改善。
   本 TODO（分解能）と同じ側の目的。
4. **マイクロフォニクス/HV 過渡の異常検出**（autoencoder）: 運用の掃除係。

**Si 的物理が唯一残る場所**: 結晶内部の光核反応 Ge(γ,p)/(γ,α)。生成荷電粒子は超局所・高密度
付着で、Ge でも既知の**プラズマ遅延**が立ち上がりに出る → 「内部反応イベント vs 通常 γ」弁別は
Si 流 PSA の親戚。ニッチだが ELI-NP らしい内部バックグラウンド研究。

インフラは §9.4 と完全共用（同じ capture run、CNN 系、delila2root→PyTorch）。250 MS/s /
14-bit は GERDA PSD（100 MS/s）より良いサンプリングで、データ品質面の障害なし。

### 9.6 文献アンカー（2026-07-13 調査）
