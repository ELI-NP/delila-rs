...
**Updated:** 2026-05-05（PHA2 Phase 1〜5 + Phase 4.5 完了。残は校正源テスト (ハード待ち) のみ。前半: 5/4 後半の 4 silent-failure バグ (decoder truncation 誤判定 / case-insensitive cache miss / UI 隠蔽 clamp / per-probe is_signed hardcode) → 5/5 再発防止 hardening (cache-miss を `info!` + `no_cache` カウンタ + 2 regression tests、`pha2_simple_test` → `caen_simple_test --firmware`、decoder hot-path heuristic 禁止 policy を `decoder/mod.rs` doc + CLAUDE.md に明文化、`pha2_56.json` revert + signed-probe 検証用 別ファイル化)。後半 (Phase 4.5): `EventData.Waveform` に `analog_probe_type[2]` + `digital_probe_type[4]` 追加 (u8 PHA2-canonical encoding + `#[serde(default)]` で BC + `UNKNOWN_PROBE_TYPE=0xFF` sentinel) → PHA2 decoder で wf-extras header (bits[2:0] analog + bits[15+4N:12+4N] digital) パース → 他 FW は Unknown 出力 → `delila_to_root` に 6 TBranch 追加 (BC 確認: 既存 `.delila` 51300 events 読める) → Frontend `Waveform` interface 拡張 + `probeLabels` computed signal + Tune Up/Live 両方のラベルを "A0: TimeFilter" 等に動的化。562 tests (新規 1)、clippy 緑、`npm run build` 緑 + `dist/` 同梱）
