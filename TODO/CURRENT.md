...
**Updated:** 2026-05-04（PHA2 FW integration Phase 1 + Phase 2 + Phase 3 ほぼ完了 — 物理パルサー入力 FWHM 0.101%、Decoder truncation resync 修正(10 unit test PASS)、apply ループバック検証、sweep スクリプト堅牢化、`.delila` → ROOT (`delila_to_root` 新 binary, FWHM 6.13 ≈ Monitor fit 6.4 で一致)。Phase 3 残: 校正源テストのみ(ハード準備待ち)。Phase 4 (UI) 着手待ち。Commits: fb8f66e, 512368c, a3aa0be, e423692）
...
|----------|------|--------|---------|
| **0** | [51_pha2_integration.md](51_pha2_integration.md) | **🚧 Phase 1 + Phase 2 + Phase 3 ほぼ完了 (2026-05-04)、校正源のみ残、Phase 4 着手待ち** | DPP-PHA on VX2730 (SN:52622 @ 172.18.4.56) 統合。Phase 1 = 配線 (FirmwareType/SourceType/DecoderKind + PHA2 to_caen_parameters ブランチ + group_input_delay plumb + dummy decoder + config files)。Phase 2 = 本物 decoder (~480 行 + 9 unit test)、TestPulse 10 kHz で 9992 events/s 100% 取得 + 487k events `.delila` 記録。Phase 3 = ① 物理パルサー入力 FWHM/Center 0.101% (Sigma=2.7) ② **Decoder bug 発見・修正** — FW が bad state で waveform を truncate するが wf_size word は嘘の値を保つ → decoder が次 event のバイトを sample として誤読してアグリゲート全体 desync。fault-dump 機能 + sample loop 中の wf_header pattern 検出で resync する fix を実装、新規 unit test 含め 10/10 PASS、実機ストレステストで 12 件の truncation を正常回復 (`fb8f66e`) ③ FW state スタック現象 (rapid Configure cycles で発生、数分 idle で自然回復) 原因究明・実証 ④ Throughput 実測 (50 kHz 100% / 100 kHz 73% / 500 kHz/200 samples で decoder 0 errors @ 147k/s) ⑤ apply config loopback verify (Gemini #5、`512368c`、set→get→log で FW 丸めを検出、21 unit test PASS、実機 0 mismatches) ⑥ Throughput sweep 堅牢化 (cool-down + auto-retry + healthy-pct 検出、`a3aa0be`) ⑦ `.delila` → ROOT 出力 (`delila_to_root` 新 binary、`e423692`、PHA2 ch0 5950 events で FWHM 6.13 確認、Monitor Gauss fit 6.4 と一致)。Phase 3 残: 校正源テストのみ(ハード準備待ち)。Phase 4 = UI (Angular) + EventData probe-type 拡張 |
