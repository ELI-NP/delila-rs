...
**Updated:** 2026-05-06（**Refactor Sprint Phase 2 R-D6 + R-D7 + R-D1/D2 + R-C1 + R-C2 + R-X1 + R-P5 完了** [TODO 52](52_refactor_sprint_2026-q2.md) — decoder family を zero-cost generic でまとめ、 reader/mod.rs を分割、 add_channel_params をテーブル駆動化、 channel override merge を proc macro 化、 production unwrap を audit、 digitizer route helper を抽出。 **R-D6 (PSD1/PHA1)** = 3 PR、 family -1531 行。 **R-D7 (PSD2/PHA2)** = 3 PR、 family -680 行。 **R-D1/D2 (read_loop split)** = 1 PR (`1059709`)、 reader/mod.rs -888 行。 **R-C1 (channel param tables)** = 1 PR (`aff8ccd`)、 add_channel_params 497→35 行 (-93%)、 digitizer.rs -451 行、 +channel_param_tables.rs 360 行 (4 const テーブル合計 121 entries、 non-capturing closure → fn pointer coercion で const 化)。 **R-C2 (MergeOverride derive)** = 1 PR (`a7bb0d2`)、 新 workspace crate `delila-derive` で `#[derive(MergeOverride)]` proc macro、 `merge_field!` ~100 行を `merge_from(ov)` 1 行に、 silent override drop バグ 3 件 (`amax`/`trigger_edge`/`trigger_threshold_v`) を構造的に fix、 digitizer.rs -103 行。 **R-X1 (unwrap audit)** = 1 PR (`007b23f`)、 35 件 production unwrap → 26 件 (許容のみ)、 Production `Result::unwrap()` 0 件達成、 9 件 fix のうち 2 件は `read_error_since` を `get_or_insert_with` で構造解消。 **R-P5 (digitizer route helpers)** = 1 PR (`43c3960`)、 6 helper で digitizer.rs 7 ハンドラ + run.rs 1 ハンドラの重複統合、 digitizer.rs 983→953 (handler bodies は -140、 helpers が +110)、 run.rs -8。 **累計 decoder -2211 行 + reader/mod.rs -888 行 + digitizer.rs -554 行 + operator/routes -38 行**。 574 tests pass、 clippy clean、 公開 API 完全互換。 残 Phase 2: R-P3/P4）
