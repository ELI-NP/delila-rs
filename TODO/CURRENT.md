...
**Updated:** 2026-05-05（PHA2 Phase 1〜5 + Phase 4.5 完了済 (校正源テスト ハード待ち)、 5/5 後半 = ① Ch Trigger Mask UI fix (commit `c45f016`、 popover + CDK Overlay、空 enum dropdown → checkbox UI、 PSD2/PHA2/AMax + PSD1/PHA1 coinc_mask 共通) ② FELib stuck -15 race fix (commit `daeb444`、`effective_state` 両方向 hw 真値化、Stop+Apply race 解消、 6 regression test、Open Issue → Resolved) ③ **Refactor Sprint 2026-Q2 計画策定** [TODO 52](52_refactor_sprint_2026-q2.md) + [clean_architecture_evaluation.md](../clean_architecture_evaluation.md) (28 candidates、3 phase 11 週間、 6 確定 decisions: dev-tools feature-gate / trait+generic / 一枚岩 / quick benchmark / daily merge / 旧 eval 削除維持)。来週 5/12〜 ランは March bin 固定、refactor は 5/12 着手 → 7/3 freeze → 7/24 次実験まで stabilize 3 週）
...
|----------|------|--------|---------|
| **0** | [52_refactor_sprint_2026-q2.md](52_refactor_sprint_2026-q2.md) | **📋 計画完了 (2026-05-05 revised late evening)、5/12 着手予定** | post-PHA2 重複削減 + **component system 維持** で構造整理 (D7、 同日朝の Pure 一枚岩 D3 を撤回)。29 candidates (R-P8 Component lifecycle + R-X3 ZMQ boundary benchmark 含む) / 3 phase / 11 週間。来週 March bin ラン中も並行可 (Phase 1 mechanical cleanup は動作不変)。Phase 1 Week 1 で **R-X3 baseline benchmark** (ZMQ 境界 cost を計測、 後で post-refactor と差分比較)、Phase 2 末 (Week 8、 〜2026-07-03) で baseline freeze、Phase 3 で **delayed reader split (R-D3/D5) + R-P8 lifecycle 統一 + R-X3 post-refactor + 7/24 実験前 stabilize**。詳細は [clean_architecture_evaluation.md](../clean_architecture_evaluation.md) (file:line citation + 工数/リスク/Impact、 §Changelog で revision 経緯)。Gemini Flash の `REFACTORING_SUGGESTIONS.md` (高レベル提案) も統合済 (R-X1/R-X2) |
