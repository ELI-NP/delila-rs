...
**Status:** ✅ **Phase 1 + Phase 2 完了 (2026-05-06 〜 2026-05-07、 全 13 + 10 = 23 項目 landed)**
...
- [x] **R-P3**: Operator `AppState` RwLock fields → 部分的に atomic + DashMap (`42acf88`、 2026-05-06)
  - **`tuneup_mode: RwLock<bool>` + `tuneup_digitizer_id: RwLock<Option<u32>>`** → `TuneupState (AtomicU64)` で 1 個の atomic に統合。 encoding: `u64::MAX` = inactive、 それ以外 = `Some(value as u32)`。 `enter(id)` / `exit()` / `is_active()` / `snapshot() -> (bool, Option<u32>)` の API。 **副次効果**: 旧コードの 2 ロック逐次更新 (例: `tuneup_mode = true` してから `tuneup_digitizer_id = Some(x)`) が non-atomic だった race を構造的に解消、 GET /api/status hot path (frontend が 1s 周期 poll) が lock-free atomic load 1 回に。 5 unit tests で encoding を pin (特に u32::MAX round-trip — sentinel と衝突しないことを確認)
  - **`digitizer_configs: RwLock<HashMap<u32, DigitizerConfig>>`** → `DashMap<u32, DigitizerConfig>`。 per-shard lock で `get(id)` / `insert(id, c)` が id 単位でしか競合しない、 全 12 call site (digitizer.rs 8 + status.rs 3 + tuneup.rs 3 + helper 1) から `.read().await` / `.write().await` 撤去、 `require_digitizer_config` / `reload_digitizer_configs` が sync 化
  - **設計注意**: DashMap shard guard (`Ref<'_, K, V>`) は `!Send` なので `.await` を跨いで保持できない。 全 caller は `.get(&id).map(|r| r.value().clone())` か `let cfg = match ... { Some(c) => c, None => continue }` で値を取り出してから `.send_command(...).await`。 lifetime で compile-time に強制されるので runtime panic にはならない
  - **当初 plan の "12 fields"** は 6 RwLock + 1 Mutex の数え方だった (clean_arch_eval `§1.2`)。 残 4 RwLock (`current_run` / `emulator_settings` / `monitor_layout`) は run start/stop or 設定編集時にしか write されず、 hot read path も無いので RwLock 維持が正解。 `tuneup_apply_lock: Mutex<()>` は serialization lock (semantic、 データを持たない) なのでそのまま維持
  - `build_channel_registrations_from()` のシグネチャ `&HashMap → &DashMap` 変更、 2 caller (status.rs / tuneup.rs) 同時更新
  - **結果**: 574 → 579 tests pass (+5)、 clippy clean、 公開 API 不変 (`SystemStatus` レスポンスの `tuneup_mode` / `tuneup_digitizer_id` field は維持、 内部表現だけが変わった)
- [x] **R-P4**: settings panel SCSS pattern + NotificationService migration (`111b9fd`、 2026-05-07)
  - 3 settings components (digitizer / emulator / event-builder) の共通 SCSS class (`.form-grid` / `.action-buttons` / `.info-card` / `.info-banner` / `.section-title` / `.hint-text` / `.form-row` / `mat-form-field { width: 100% }`) を per-component `<styles>` から `web/operator-ui/src/styles.scss` の global `.settings-panel` scope に集約。 各 component は host element に `class="settings-panel"` 1 行付与で取り込み、 digitizer-settings は tab body 内の row gap を狭くする `.form-grid` 1 override のみ意図的に retain
  - 同 PR で 12 `MatSnackBar.open()` call site (digitizer 5 / emulator 4 / event-builder 3) を `NotificationService.success() / .error() / .warning()` (既存 `services/notification.service.ts`、 status / run / monitor flow で既に canonical) に migrate、 3 component から `MatSnackBarModule` import + `MatSnackBar` ctor injection を撤去 → UI 全体で **1 通知サーフェス** に統一
  - **当初 plan の generic base class** (`AbstractSettingsPanel<T>` 等) は採用せず: 3 components の shape が genuinely different (single-card / multi-tab+experiment / multi-tab+per-id+channel-table) で、 generic 化が可読性を下げる。 R-P5 で generic factory を採用しなかった同じ判断
  - **結果**: digitizer-settings.component.ts -32 / emulator-settings.component.ts -44 / event-builder-settings.component.ts -15 / styles.scss +66 = net -25 行、 ng test 68/68 pass、 ng build clean、 cargo test 579 pass、 clippy clean、 dist/ 再ビルド済み (CLAUDE.md "Frontend Deployment Policy" 遵守)。 **Phase 2 全 10 項目 (R-D6/D7/D1/D2/C1/C2/X1/P5/P3/P4) landed**
...
**Phase 2 全 10 項目 landed (2026-05-07)**: R-D6 / R-D7 / R-D1/D2 / R-C1 / R-C2 / R-X1 / R-P5 / R-P3 / R-P4。 reader/mod.rs ~4000 → 3112 行 (R-D3/D5 で Phase 3 で更に縮める予定)、 config/digitizer.rs 3080 → 2526 行。
