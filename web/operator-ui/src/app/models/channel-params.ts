...
import { AMAX_PARAMS_BY_CATEGORY } from './amax-generated';
...
export type ChannelCategory =
  ...
  | 'coincidence'
  | 'waveform'
  // Open enum: AMax firmware can declare new categories (e.g. `debug`) via
  // fw_params.json + codegen. `(string & {})` keeps autocomplete on the
  // named members while accepting any future category name.
  | (string & {});
...
/** Human-facing label for the well-known categories (used as `mat-tab label`). */
...
/** Display label for a category, falling back to a capitalised form for any
 *  new firmware category that has no explicit entry above. Keeps new AMax
 *  tabs readable without a hand-edit. */
export function channelCategoryLabel(category: string): string {
  return CHANNEL_CATEGORY_LABELS[category] ?? category.charAt(0).toUpperCase() + category.slice(1);
}
...
/** Ordered list of the categories a firmware actually exposes: the well-known
 *  categories it defines (in canonical order) followed by any firmware-specific
 *  extras (e.g. a future AMax `debug` category from codegen) that aren't in the
 *  canonical list. Used by both the Settings and Tune Up panels so a new
 *  category renders a tab without any hand-edit. */
export function getFirmwareCategories(fw: FirmwareType): ChannelCategory[] {
  const cats = CATEGORY_PARAMS[fw];
  if (!cats) return [];
  const known = CHANNEL_CATEGORIES.filter((c) => cats[c] !== undefined);
  const extra = Object.keys(cats).filter(
    (c) => !(CHANNEL_CATEGORIES as readonly string[]).includes(c),
  );
  return [...known, ...extra];
}
...
export function getAllChannelParams(fw: FirmwareType): ChannelParamDef[] {
  const cats = CATEGORY_PARAMS[fw];
  if (!cats) return [];
  return getFirmwareCategories(fw).flatMap((c) => cats[c] ?? []);
}
