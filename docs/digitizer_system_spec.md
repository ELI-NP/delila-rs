...
|-------|----------|---------|----------|--------|
| VX2730 | DPP-PSD2 | CAEN_FELib | 32 | **Production (MVP)** |
| VX2730 | DPP-PHA2 | CAEN_FELib | 32 | **Production (2026-05)** ※2 |
| VX2730 | DPP-OPEN (AMax) | CAEN_FELib | 32 (ch0/1 active) | **Production (2026-04)** |
| VX2745 | DPP-PSD2 | CAEN_FELib | 64 | Future |
| VX2740 | DPP-PSD2 | CAEN_FELib | 64 | Future |
| x725 | DPP-PSD1 | CAEN_FELib | 8/16 | **Production** |
| x730 | DPP-PSD1 | CAEN_FELib | 8/16 | **Production** |
| x725 | DPP-PHA1 | CAEN_FELib | 8/16 | **Production** |
| x730 | DPP-PHA1 | CAEN_FELib | 8/16 | **Production** |
| VX1743 | SAMLONG (Standard) | CAEN_FELib + DPP | 16 | **Production (2026-04)** ※1 |

**※1 VX1743について:**
- SAMLONG ASIC + ホスト側 RolloverTracker で 40-bit TDC + CFD soft fine-time
- DPP-CI 経路は撤退済み(2026-04-20、設計バグ + TimeTag 欠如)
- 95 分連続ランで rollover 通過確認、本番投入済(SN:25 @ 172.18.4.147)

**※2 PHA2 について(2026-05-04):**
- VX2730 に DPP-PHA firmware を flash した運用形態。SN:52622 @ 172.18.4.56 で稼働
- 物理パルサー入力で FWHM/Center = 0.101%(Sigma=2.7、Center=6327.6)を達成
- Decoder は `src/reader/decoder/pha2.rs`(独立実装、PSD2 と共通化せず — KISS + Rule of Three)
- **Known FW quirk**: rapid Configure cycles で稀に低 rate stuck 状態に陥る(数分 idle で自然回復、クレート電源リセットで強制回復)。Run 開始後は ADC スペクトラムを必ず目視確認すること
- Throughput: 単発 fresh-start で 50 kHz @ 100% delivery、500 kHz/200 samples で decoder 0 errors
- Detail: [TODO/51_pha2_integration.md](../TODO/51_pha2_integration.md)
