...
pub fn channel_writes(
    ...
    if let Some(v) = config.selector_wave {
        writes.push((REG_SELECTOR_WAVE, v, "selector_wave"));
    }
    ...
    if let Some(v) = config.pretrigger_trap {
        writes.push((REG_PRETRIGGER_TRAP, v, "pretrigger_trap"));
    }
    if let Some(v) = config.pretrigger_amax {
        writes.push((REG_PRETRIGGER_AMAX, v, "pretrigger_amax"));
    }
    ...
    if let Some(v) = config.shap_bl_hold {
        writes.push((REG_SHAP_BL_HOLD, v, "shap_bl_hold"));
    }
    writes
}
...
pub fn board_writes(
    ...
    if let Some(v) = config.enable_acq {
        writes.push((BOARD_REG_ENABLE_ACQ, v, "enable_acq"));
    }
    writes
}
