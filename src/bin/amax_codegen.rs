...
struct Cli {
    ...
    /// Channel-page base address (word units, ch0)
    #[arg(long, default_value_t = 0x800000)]
    page_base: u32,

    /// Word stride between consecutive channel pages
    #[arg(long, default_value_t = 0x40000)]
    page_stride: u32,
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Pull just the channel-0 registers — that's our schema. Channel ≥1 is a copy.
    let ch0_prefix = "page_amax_energy_0/";
    let mut ch0: Vec<&Register> = register_file
        .registers
        .iter()
        .filter(|r| r.path.starts_with(ch0_prefix))
        .collect();
    ...
    for r in &ch0 {
        let name = r.path.trim_start_matches(ch0_prefix).to_string();
        if fw_params
            .readonly_patterns
            .iter()
        ...
        resolved.push(ResolvedReg {
            ...
            word_offset: r.address - cli.page_base,
            bits,
...
fn emit_rust_struct(
    ...
    out.push_str(
        "//!\n\
         ...
         //! `0x800000 + channel * 0x40000 + REG_<NAME>` (word units).\n\
         \n\
...
fn emit_rust_registers(
    ...
    out.push_str(
        "\x20\x20\x20\x20\x20\x20\x20\x20];\n\
         ...
         \x20\x20\x20\x20\x20\x20\x20\x20// ch0 base byte addr = PAGE_BASE * 4\n\
         \x20\x20\x20\x20\x20\x20\x20\x20assert_eq!(channel_register_byte_addr(0, 0), PAGE_BASE * 4);\n\
         \x20\x20\x20\x20\x20\x20\x20\x20// ch1 = ch0 + PAGE_STRIDE * 4 (byte units)\n\
