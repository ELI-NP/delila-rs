...
def build_l1_or_of_triggers(elifant_ch: list) -> dict:
    ...
    Threshold cuts (formerly `--trigger-min-adc`) are not generated here —
    SPEC v0.7 § 1.4 forbids per-channel energy gates inside the EB.
    Apply them in the analysis macro instead.
    """
    definitions = []
    or_inputs = []
    for mod in elifant_ch:
