...
- **2026-05-05 late evening**: D8 (ZMQ 境界 benchmark) を **optional → commit に格上げ**、 R-X3 として §2.4 にカタログ化。 baseline (Phase 1 Week 1) + post-refactor (Phase 3 Week 11) の 2 点測定で「ZMQ 境界 cost が実際どれくらいか」を数字で答える。 結果は `docs/plans/zmq_boundary_cost_2026-Q2.md`。 sprint 設計は引き続き gate しない (R-X3 の値次第で計画は変わらない)。
- **2026-05-05 evening**: D3 (Pure 一枚岩) と D4 (gating benchmark) を **撤回**。 component system 維持で進めると決定 (新 D7)、ZMQ 境界 benchmark は optional に格下げ (新 D8、同日中に R-X3 へ昇格)。 §3 Phase 3 を「monolithic 化」から「component lifecycle 統一 + 延期 reader split + stabilize」に書き換え、 §2.5 R-A* を削除、 新 R-P8 (Component lifecycle trait) を §2.3 に追加。 詳細は [TODO 52](TODO/52_refactor_sprint_2026-q2.md) 「方針 (revised)」参照。
...
| Phase | 期間 | 性格 | 対象 R |
|---|---|---|---|
| 1. Mechanical | Week 1–3 | 動作不変、低リスク、PR を寝かせ可能 | R-D4, R-D8, R-D9, R-D10, R-C3, R-C4, R-C5, R-C6, R-P1, R-P2, R-P7, R-X2, **R-X3 baseline** |
| 2. Structural | Week 4–8 | 中リスク、テスト依存、 高 impact | R-D1, R-D2, R-D6, R-D7, R-C1, R-C2, R-P3, R-P4, R-P5, R-X1 |
| 3. Component hardening + stabilize | Week 9–11 | delayed reader split + lifecycle 統一 + 7/24 実験前検証 | R-D3, R-D5, R-D11, R-D12 (optional), R-P6, **R-P8 (new)**, hardware dry run, **R-X3 post-refactor** |
...
| **R-C2** | `merge_field!` 65+ fields の手動展開を `#[derive(MergeOverride)]` proc macro に (`a7bb0d2`、 2026-05-06 完了)。 副次に silent override drop バグ 3 件 (`amax`/`trigger_edge`/`trigger_threshold_v`) を構造的に fix。 新 workspace crate `delila-derive/` | M | 低 | 中 | 1 |
...
**Production binary (15)**: emulator, data_sink, merger, controller, reader, recorder, monitor, event_bridge, event_builder, online_event_builder, node_agent, operator, recover, event_dump, delila2root
...
| **R-P3** | Operator `AppState` lock-free 化 (`42acf88`、 2026-05-06 完了)。 `tuneup_mode + tuneup_digitizer_id` 2 RwLock を `TuneupState (AtomicU64)` に統合 (non-atomic race 解消、 GET /api/status lock-free に)、 `digitizer_configs: RwLock<HashMap>` → `DashMap`、 全 12 call site から read/write await 撤去。 残 4 RwLock + 1 Mutex は維持 (write rare + 非 hot read path) | M | 中 | 中 | 1 |
| **R-P4** | digitizer/event-builder/emulator settings の base settings panel パターン化 | M | 中 | 中 | 2–3 |
| **R-P5** | `digitizer.rs` ハンドラの重複削減 (`43c3960`、 2026-05-06 完了)。 generic factory plan は採用せず、 6 single-purpose helper (`require_digitizer_repo` / `require_digitizer_config` / `reject_path_id_mismatch` / `resolve_config_path` / `write_digitizer_config` / `mongodb_unavailable`) の合成方式へ切り替え。 digitizer.rs 983→953、 run.rs -8。 7 ハンドラ + run.rs 1 ハンドラを統合 | M | 中 | 中 | 1 |
...
| ID | Title | 工数 | リスク | Impact | PR数 |
|---|---|---|---|---|---|
| **R-X1** | Production `unwrap()` audit & reduction (`007b23f`、 2026-05-06 完了)。 35 → 26 件 (-9)、 残は仕様許容: 22 件 `Mutex::lock().unwrap()` + 4 件 doc-comment example。 **Production `Result::unwrap()` 0 件達成**。 構造解消 2 件 (`read_error_since` Option/unwrap → `get_or_insert_with`)、 `expect("reason")` 化 7 件 | M | 低 | 中 | 1 |
| **R-X2** | `cargo audit` (security advisories) + `cargo outdated` (依存更新可視化) を pre-commit / CI に追加 | S | 低 | 低 | 1 |
| **R-X3** | (NEW、 D8 commit) ZMQ 境界 cost benchmark の **baseline (Phase 1 Week 1) + post-refactor (Phase 3 Week 11) の 2 点測定**。 新 bin `zmq_boundary_bench` で emulator → reader → merger → recorder のパイプラインを 10k / 100k / 1M ev/s で流し、 (a) end-to-end Throughput、 (b) Latency (Stop→EOS / Apply→Configured / Start→first event)、 (c) per-boundary encode/decode 時間、 (d) per-event bytes、 (e) CPU usage を計測。 結果は `docs/plans/zmq_boundary_cost_2026-Q2.md` に baseline / post / 差分の 3 セクションで残す | M | 低 | 中 | 2 (baseline + post) |

**経緯**:
- R-X1 / R-X2: Gemini Flash refactoring suggestion (`REFACTORING_SUGGESTIONS.md`, 2026-05-05) で指摘された 2 件、 本 audit が拾い損ねていたので catalog に追加。
- R-X3: 2026-05-05 evening、 D8 を「optional/低優先」から **「commit、 2 点測定」** に格上げした際に新規 catalog 化。 sprint 設計を gate しないが、 「ZMQ 境界 cost が実際どれくらいか」という長年の宿題に数字で答えるための位置付け。
...
| Week | タスク | 完了基準 |
|---|---|---|
| 1 | R-D4, R-D8, R-D9, R-C4, **R-X2** (cargo audit + outdated を CI に)、**R-X3 baseline** (zmq_boundary_bench 新設 + 重複削減着手前のクリーンな状態で計測) | 568 tests 緑、clippy clean、CI で audit/outdated 走る、 `docs/plans/zmq_boundary_cost_2026-Q2.md` に Baseline セクション完成 |
...
| 11 | **R-X3 post-refactor** (Phase 1 baseline と同条件で再測定 → `docs/plans/zmq_boundary_cost_2026-Q2.md` に Post-refactor + 差分結論を追記)、 hardware-in-the-loop dry run (PSD2 / PHA2 / V1743 各 10 分以上)、回帰テスト全緑、 `git tag refactor-sprint-2026-q2-complete` |
...
| ~~D4~~ | ~~D3 の gating benchmark~~ | **撤回 (2026-05-05 evening)**。 判断 gate ではなくなったが、 計測自体の価値は残るので D8 (= R-X3) で 2 点測定として再度コミット (下記参照)。 |
...
| **D8** | ZMQ 境界 cost benchmark (NEW 2026-05-05 evening、 同日中に commit へ格上げ) | **commit、 2 点測定 (R-X3)**。 Phase 1 Week 1 で baseline を取り、 Phase 3 Week 11 で post-refactor を取る。 結果は `docs/plans/zmq_boundary_cost_2026-Q2.md`。 sprint 設計を gate しないが、 「ZMQ 境界 cost が実際どれくらいか」という長年の宿題に数字で答え、 将来意思決定 (例: monolithic 再検討、 EB 設計) のレファレンスにする。 |
...
- `docs/plans/zmq_boundary_cost_2026-Q2.md` に baseline + post-refactor の 2 点 benchmark 結果と差分結論 (R-X3)
